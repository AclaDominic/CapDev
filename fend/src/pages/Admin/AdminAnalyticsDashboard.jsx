    ...
    <div className="card h-100 border-0 shadow-sm" style={{ 
      ...
      <div className="card-body p-4">
        <div className="d-flex align-items-center justify-content-between mb-3">
          <div className="text-muted small fw-medium text-uppercase tracking-wide" style={{ 
            letterSpacing: '0.5px',
            fontSize: '0.75rem'
          }}>
            {title}
        ...
        <div className="fs-2 fw-bold mb-2" style={{ 
          color: '#1f2937',
          lineHeight: '1.2'
        }}>
          {value ?? 0}
        </div>
        <div className={"small fw-medium d-flex align-items-center " + ((change ?? 0) >= 0 ? "text-success" : "text-danger")}>
          <span className="me-1">
            {(change ?? 0) >= 0 ? '↗' : '↘'}
          </span>
    ...
    <>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0 mt-1">Monitor your clinic's performance and key metrics</p>
        </div>
                ...
                <div className="card-body pt-0">
                  <div className="list-group list-group-flush">
                    {(data?.top_services || []).map((s, index) => (
                      <div key={`${s.service_id}-${s.service_name}`} 
                           className="list-group-item border-0 px-0 py-3 d-flex justify-content-between align-items-center"
                           style={{ 
                             borderBottom: index < (data?.top_services || []).length - 1 ? '1px solid rgba(0,0,0,0.05)' : 'none'
                           }}>
                        <div className="d-flex align-items-center">
                        ...
                        <div className="d-flex align-items-center">
                          <strong className="me-3 fs-6" style={{ color: '#1f2937' }}>{s.count}</strong>
                          <span className={`badge ${(s.pct_change ?? 0) >= 0 ? "bg-success" : "bg-danger"} px-2 py-1`} 
                  ...
                  {topServiceInsight && (
                    <div className="mt-3 p-3 rounded-3" style={{ backgroundColor: 'rgba(16, 185, 129, 0.05)' }}>
                      <small className="text-muted">{topServiceInsight}</small>
                    </div>
                  )}
                </div>
              </div>
                ...
                {(data?.alerts || []).length === 0 ? (
                  ...
                  <div className="list-group list-group-flush">
                    {(data?.alerts || []).map((a, idx) => (
                      <div key={idx} className={`list-group-item border-0 px-0 py-3 d-flex align-items-start ${
